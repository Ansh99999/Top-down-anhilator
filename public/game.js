...
function renderGarage() {
    ...
    // Capture focus index before destroy
    let focusIdx = -1;
    if (document.activeElement && grid.contains(document.activeElement)) {
        focusIdx = Array.from(grid.children).indexOf(document.activeElement);
    }
    ...
    // Restore focus
    if (focusIdx >= 0 && grid.children[focusIdx]) {
