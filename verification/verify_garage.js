...
(async () => {
  ...
  try {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        // Allow <button> tag OR role="button"
        if (tagName !== 'button' && role !== 'button') console.error(`Card ${i} missing role="button" (and is not a <button>)`);

        // Allow <button> (implicit tabindex) OR tabindex="0"
