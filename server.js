...
class GameInstance {
  ...
  resolveCollision(p, radius) {
    ...
    // ⚡ Bolt: Squared distance check optimized
  ...
  update() {
    ...
    for (let id in this.players) {
      ...
      if (p.isShooting && p.fireCooldown <= 0 && !p.overload) {
        ...
        // Passives
        if (def.passives.includes('SPEED_DAMAGE') || def.passives.includes('SIEGE_MODE')) {
            let vx = p.vx || 0; let vy = p.vy || 0;
  ...
  updateBullets(){
      ...
      for (let i = this.destructibles.length - 1; i >= 0; i--) {
        if (this.destructibles[i].hp <= 0) {
          ...
          for (let id in this.players) {
              let p = this.players[id];
              let dx = p.x - d.x; let dy = p.y - d.y;
              if (dx*dx + dy*dy < 10000) p.hp -= 30; // 100^2
          }
          this.enemies.forEach(e => {
  ...
  updateEnemies(){
      ...
      this.enemies.forEach(e => {
        ...
        for (let id in this.players) {
          ...
          let dSq = dx*dx + dy*dy;
          if (p.type === 5) dSq /= 4; // Tank draws aggro (distance halved = squared distance / 4)
                    ...
                    if (e.hp <= 0) {
                        ...
                        if (e.modifiers.includes('EXPLOSIVE')) {
                            ...
                            for (let id in this.players) {
                                let p = this.players[id];
                                let dx = p.x - e.x; let dy = p.y - e.y;
                                if (dx*dx + dy*dy < 22500) p.hp -= 40; // 150^2
                            }
                        }
            ...
            if(dx*dx + dy*dy < combinedR * combinedR){
                ...
                if(e.type === 8) { // Suicider Explode on Contact
                    ...
                    for (let pid in this.players) {
                        ...
                        let pdx = pp.x - e.x; let pdy = pp.y - e.y;
                        if (pdx*pdx + pdy*pdy < 22500) pp.hp -= 40; // 150^2
                    }
                    break;
                }
            }
