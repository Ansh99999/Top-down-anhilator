    ...
    for (let id in this.players) {
      ...
      if (p.isShooting && p.fireCooldown <= 0 && !p.overload) {
        ...
        if (def.passives.includes('SIEGE_MODE')) {
             let spdSq = (p.vx || 0)**2 + (p.vy || 0)**2;
             if (spdSq < 0.25) dmg *= 1.3;
        }
                ...
                if (dx*dx + dy*dy < hitR*hitR) {
                    ...
                    if (e.hp <= 0) {
                        ...
                        if (e.modifiers.includes('EXPLOSIVE')) {
                            ...
                            for (let id in this.players) {
                                let px = this.players[id].x - e.x; let py = this.players[id].y - e.y;
                                if (px*px + py*py < 22500) this.players[id].hp -= 40; // 150^2
                            }
                        }
            ...
            if(dSq < hitR*hitR){
                ...
                if(e.type === 8) { // Suicider Explode on Contact
                    ...
                    for (let pid in this.players) {
                        let px = this.players[pid].x - e.x; let py = this.players[pid].y - e.y;
                        if (px*px + py*py < 22500) this.players[pid].hp -= 40; // 150^2
                    }
                    break;
                }
            }
