  ...
  updateEnemies(){
      ...
      this.enemies.forEach(e => {
        ...
        for (let id in this.players) {
          ...
          let dSq = dx*dx + dy*dy;
          if (p.type === 5) dSq /= 4; // Tank draws aggro (distance halved -> squared distance quartered)
          if (dSq < minDSq) { minDSq = dSq; target = p; }
        }

        e.stateTimer--;
        if (e.stateTimer <= 0) {
          if (minDSq < 40000 && e.type === 1) e.state = 'flee';
          else if (minDSq < 160000 && e.type === 1) e.state = 'strafe';
          else if (minDSq < 1000000) e.state = 'chase';
          else e.state = 'wander';
        ...
        if (target && minDSq < 2250000) { // 1500^2
          ...
          if (e.state === 'chase') {
            ...
            if (Math.random() < 0.05 && minDSq < 250000)
            ...
            if(dx*dx + dy*dy < minDist*minDist){
                ...
                if(e.type === 8) { // Suicider Explode on Contact
                    ...
                    for (let pid in this.players) {
                        ...
                        let dx = pp.x - e.x; let dy = pp.y - e.y;
                        if (dx*dx+dy*dy < 22500) pp.hp -= 40;
                    }
                    break;
                }
            }
