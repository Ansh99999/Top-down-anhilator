...
class GameInstance {
  ...
  updateBot(bot){
      ...
      if(target && minDSq<360000){ // 600^2
        ...
        let dToOwnerSq=(owner.x-bot.x)**2 + (owner.y-bot.y)**2;
  ...
  updateEnemies(){
      ...
      this.enemies.forEach(e => {
        ...
        for (let id in this.players) {
          ...
          let dSq = dx*dx + dy*dy;
          if (p.type === 5) dSq /= 4; // Tank draws aggro (distance / 2 => squared distance / 4)
      ...
      for (let j = this.enemies.length - 1; j >= 0; j--) {
        ...
        for(let id in this.players){
            let p = this.players[id];
            let dx = p.x - e.x; let dy = p.y - e.y;
            let rSum = (e.radius || 32) + 20; // Default to 32 if undefined
