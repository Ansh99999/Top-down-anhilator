...
class GameInstance {
  ...
  resolveCollision(p, radius) {
    ...
    // ⚡ Bolt: Optimize map boundary check using squared distance
    let dSq = p.x * p.x + p.y * p.y;
    let maxDist = mapR - radius;
    if (dSq > maxDist * maxDist) {
      let ang = Math.atan2(p.y, p.x);
      p.x = Math.cos(ang) * maxDist;
      p.y = Math.sin(ang) * maxDist;
    }
  ...
  updateBullets(){
      ...
      for (let i = this.destructibles.length - 1; i >= 0; i--) {
        if (this.destructibles[i].hp <= 0) {
          ...
          // Area Dmg (100^2 = 10000)
          for (let id in this.players) {
              let p = this.players[id];
              let dSq = (p.x-d.x)*(p.x-d.x) + (p.y-d.y)*(p.y-d.y);
              if (dSq < 10000) p.hp -= 30;
          }
          this.enemies.forEach(e => {
              let dSq = (e.x-d.x)*(e.x-d.x) + (e.y-d.y)*(e.y-d.y);
              if (dSq < 10000) e.hp -= 50;
          });
  ...
  updateEnemies(){
      ...
      this.enemies.forEach(e => {
        ...
        for (let id in this.players) {
          ...
          let dSq = dx*dx + dy*dy;
          let effectiveDSq = dSq;
          if (p.type === 5) effectiveDSq /= 4; // Tank draws aggro (d/2)^2 = d^2/4
          if (effectiveDSq < minDSq) { minDSq = effectiveDSq; target = p; }
        }
      ...
      for (let i = this.bullets.length - 1; i >= 0; i--) {
        ...
        if (b.owner === 'enemy') {
            for (let id in this.players) {
                let p = this.players[id];
                let dSq = (b.x-p.x)*(b.x-p.x) + (b.y-p.y)*(b.y-p.y);
                if (dSq < 400) { // 20^2
                    let dmg = b.damage;
                    let def = VEHICLE_DEFS[p.type] || VEHICLE_DEFS[0];
                ...
                if (dSq < hitR * hitR) {
                    ...
                    if (e.hp <= 0) {
                        ...
                        if (e.modifiers.includes('EXPLOSIVE')) {
                            ...
                            for (let id in this.players) {
                                let p = this.players[id];
                                let expDSq = (p.x-e.x)*(p.x-e.x) + (p.y-e.y)*(p.y-e.y);
                                if (expDSq < 22500) p.hp -= 40; // 150^2
                            }
                        }
      ...
      for (let j = this.enemies.length - 1; j >= 0; j--) {
        ...
        for(let id in this.players){
            let p = this.players[id];
            let dSq = (p.x-e.x)*(p.x-e.x) + (p.y-e.y)*(p.y-e.y);
            let minDist = e.radius + 20;
            if(dSq < minDist * minDist){
                ...
                if(e.type === 8) { // Suicider Explode on Contact
                    ...
                    for (let pid in this.players) {
                        ...
                        let expDSq = (pp.x-e.x)*(pp.x-e.x) + (pp.y-e.y)*(pp.y-e.y);
                        if (expDSq < 22500) pp.hp -= 40; // 150^2
                    }
                    break;
                }
            }
