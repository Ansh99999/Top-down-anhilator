      ...
      for (let i = this.destructibles.length - 1; i >= 0; i--) {
        if (this.destructibles[i].hp <= 0) {
          ...
          for (let id in this.players) {
              ...
              if (dx*dx + dy*dy < 10000) this.players[id].hp -= 30; // 100^2
          }
          this.enemies.forEach(e => {
              let dx = e.x - d.x; let dy = e.y - d.y;
              if (dx*dx + dy*dy < 10000) e.hp -= 50;
          });
  ...
  updateEnemies(){
      ...
      this.enemies.forEach(e => {
        ...
        for (let id in this.players) {
          ...
          let dSq = dx*dx + dy*dy;
          if (p.type === 5) dSq /= 4; // Tank aggro: d/2 -> dSq/4
                    ...
                    if (e.hp <= 0) {
                        ...
                        if (e.modifiers.includes('EXPLOSIVE')) {
                            ...
                            for (let id in this.players) {
                                ...
                                if (pdx*pdx + pdy*pdy < 22500) this.players[id].hp -= 40; // 150^2
                            }
                        }
            ...
            if(dx*dx + dy*dy < hitDist*hitDist){
                ...
                if(e.type === 8) { // Suicider Explode on Contact
                    ...
                    for (let pid in this.players) {
                        ...
                        if (pdx*pdx + pdy*pdy < 22500) this.players[pid].hp -= 40; // 150^2
                    }
                    break;
                }
            }
...
module.exports = { app, server, GameInstance, VEHICLE_DEFS };
