...
let fireRate=10; // Default
if(data.type===0) fireRate=5; // Speedie fast
if(data.type===3 || data.type===5) fireRate=20; // Heavy/Juggernaut slow
if(data.type===6) fireRate=8; // Interceptor
players[socket.id]={
id:socket.id,x:Math.random()*MAP_WIDTH,y:Math.random()*MAP_HEIGHT,angle:0,turretAngle:0,
...
socket.emit('init',{id:socket.id,players,obstacles,map:{w:MAP_WIDTH,h:MAP_HEIGHT},wave,items,structures});
...
if(p.type===5){p.hp=Math.min(p.hp+100,p.maxHp);threat+=5;} // Juggernaut: Mega Heal + Threat
if(p.type===6){p.speed=15;setTimeout(()=>p.speed=8,2000);} // Interceptor: Boost
if(p.type===7){ // Engineer: Deploy Turret
structures.push({id:Math.random(),x:p.x,y:p.y,hp:100,owner:p.id,life:1000});
}
}
setInterval(()=>{
// Adapt stats
...
// Bullet Spawning (RPM Check)
for(let id in players){
let p=players[id];
if(p.fireCooldown>0) p.fireCooldown--;
if(p.isShooting && p.fireCooldown<=0){
let dmg=p.damage*p.buffs.damage;
if(p.type===6) dmg+=p.speed; // Interceptor momentum dmg
...
let r=Math.random();
let type=0;
if(adapt.speed>6 || threat>50) type=1; // Fast players/High threat -> Shooter
if(adapt.speed<3) type=0; // Slow players -> Rusher
if(Math.random()<0.3 + threat/200) type=2; // Ambusher chance
...
if(p.type===5) d/=2; // Juggernaut attracts aggro
...
bot.turretAngle=angle;bot.isShooting=true; // Bot shoots
}else bot.isShooting=false;
}
}
// Collisions
...
if(e.hp<=0){
enemies.splice(j,1);
let shooter=players[b.owner];if(shooter){shooter.score+=10;threat+=2;}
...
if(d<30){
p.hp-=1;
if(p.type===6 && p.speed>10){e.hp-=50;if(e.hp<=0)enemies.splice(idx,1);} // Interceptor Ram
}
});
